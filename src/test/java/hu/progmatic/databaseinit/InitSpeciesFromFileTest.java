...
class InitSpeciesFromFileTest {
    ...
    class speciesTest {
        ...
        void speciesRecordTest() {
            Species speciesTest = speciesList.stream().filter(speces -> speces.getNev().equals("Fossza")).findAny().orElseThrow();
            ...
            speciesTest = speciesList.stream().filter(speces -> speces.getNev().equals("Hosszúfarkú Vidra")).findAny().orElseThrow();
        ...
        void speciesParentCladTest() {
            Species speciesTest = speciesList.stream().filter(speces -> speces.getNev().equals("Fossza")).findAny().orElseThrow();
            assertEquals("madagaszkári cibetmacskafélék családja", speciesTest.getKlad().getNev());

            speciesTest = speciesList.stream().filter(speces -> speces.getNev().equals("Kinkaju")).findAny().orElseThrow();
            assertEquals("mosómedvefélék családja", speciesTest.getKlad().getNev());
        }
    }
}